...
- [构造方法](#构造方法)
- [put 方法分析](#put-方法分析)
  - [hash(Object key) 方法](#hashobject-key-方法)
...
- 数据重复：如果有两个线程要插入的 key 是相同的，而这个 key 不存在，在向链表中写入的时候第一线程将 e 设置为了自己的 Node，而第二个线程执行到了 n.next，此时拿到的是最后一个节点，依然会将自己持有的数据插入到链表中，这样就出现了数据重复。
