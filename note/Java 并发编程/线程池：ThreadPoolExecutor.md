...
- 线程调度也会有很大的开销。线程的调度会导致上下文切换。频繁的创建和销毁线程，导致频繁的切换，从而增加处理器资源的消耗；
...
- **corePoolSize：核心线程池数大小**。线程池中所保存的线程数，包括空闲线程。当提交一个任务到线程池时，线程池会创建线程来执行任务，即使存在空闲线程也会创建新线程，等到需要执行的任务数大于该核心线程数就不再创建线程。

- **maximumPoolSize：线程池中允许的最大线程数**。

- **keepAliveTime：空闲线程保持存活的时间**。但线程池中的线程数大于 corePoolSize 时，多余的空闲线程在终止之前等待新任务的最大时间。所以，如果任务很多，并且每个任务的执行时间较短，那么可以调大时间，提高线程的利用率。
...
- shutdownNow() 方法：线程池拒接收新提交的任务，同时立马关闭线程池，线程池里的任务不再执行。
- shutdown() 方法：线程池拒接收新提交的任务，同时等待线程池里的任务执行完毕后关闭线程池。
