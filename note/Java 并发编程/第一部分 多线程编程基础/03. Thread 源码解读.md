...
所以，默认名称的格式为 ``Thread-``加上``threadInitNumber``线程初始化数量。
...
一般情况下使用默认优先级即可，不恰当的设置优先级可能导致严重的问题**（线程饥饿）**。（TODO ？？？）
...
就是因为这里设置了 thread1 为守护线程，在 thread2 和主线程都执行完了，Java 虚拟机就正常停止，thread1 守护线程就不执行了。所以：
